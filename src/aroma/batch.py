...
from aroma.sigma_only import SigmaOnlyResult, run_sigma_only_molecule
...
# A unit of sigma-only NICS_pizz work over a whole molecule (all rings, two SCFs):
# (path, molecule, rings, backend, start, stop, step, h_distance).
SomMoleculeJob = Tuple[
    Path, Molecule, List[List[int]], ShieldingBackend, float, float, float, float
]
...
def _som_molecule_job(
    job: SomMoleculeJob,
) -> Tuple[Path, List[Tuple[List[int], SigmaOnlyResult]]]:
    """Run a molecule's sigma-only NICS_pizz (two SCFs); return (path, results)."""
    path, mol, rings, backend, start, stop, step, h_distance = job
    return path, run_sigma_only_molecule(
        mol, rings, backend, None, start, stop, step, h_distance
    )
